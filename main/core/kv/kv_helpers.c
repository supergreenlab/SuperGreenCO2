...
static SemaphoreHandle_t _mutex_co2; // TODO check RAM weight of creating so many semaphores :/
static bool _co2_changed = true;
static bool _co2_undefined = true;

void reset_co2_changed() {
...
bool is_co2_undefined() {
  xSemaphoreTake(_mutex_co2, 0);
  bool v = _co2_undefined;
  xSemaphoreGive(_mutex_co2);
  return v;
}
...
static SemaphoreHandle_t _mutex_solenoid; // TODO check RAM weight of creating so many semaphores :/
static bool _solenoid_changed = true;
static bool _solenoid_undefined = true;

void reset_solenoid_changed() {
  xSemaphoreTake(_mutex_solenoid, 0);
  _solenoid_changed = false;
  xSemaphoreGive(_mutex_solenoid);
}

bool is_solenoid_changed() {
  xSemaphoreTake(_mutex_solenoid, 0);
  bool v = _solenoid_changed;
  xSemaphoreGive(_mutex_solenoid);
  return v;
}

bool is_solenoid_undefined() {
  xSemaphoreTake(_mutex_solenoid, 0);
  bool v = _solenoid_undefined;
  xSemaphoreGive(_mutex_solenoid);
  return v;
}


static int _solenoid = INT_MAX;
...
void set_solenoid(int value) {
  xSemaphoreTake(_mutex_solenoid, 0);
  if (_solenoid == value) return;
  _solenoid = value;
  _solenoid_changed = true;
  _solenoid_undefined = false;
  xSemaphoreGive(_mutex_solenoid);
}
