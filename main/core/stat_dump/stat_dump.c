...
static void stat_dump_task(void *param) {
  ...
  while(1) {
    ...
    if ((counter % 20) == 0 || is_co2_changed()) {
      value = get_co2();
      ESP_LOGI(SGO_LOG_METRIC, "@KV %s=%d", "CO2", value);
      vTaskDelay(200 / portTICK_PERIOD_MS);
      reset_co2_changed();
    }
    if ((counter % 20) == 0 || is_solenoid_changed()) {
      value = get_solenoid();
      ESP_LOGI(SGO_LOG_METRIC, "@KV %s=%d", "SOLENOID", value);
      vTaskDelay(200 / portTICK_PERIOD_MS);
